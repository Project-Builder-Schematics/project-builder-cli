// Package pathutil provides path canonicalisation and traversal-rejection
// helpers shared across feature packages.
//
...
// Canonicalise applies filepath.Abs + filepath.Clean to rawDir and
...
//
// The Op field in returned errors is "pathutil.Canonicalise" to allow callers
// to wrap with their own context if needed.
func Canonicalise(rawDir string) (string, error) {
	abs, err := filepath.Abs(rawDir)
	if err != nil {
		return "", &errs.Error{
			Code:    errs.ErrCodeInvalidInput,
			Op:      "pathutil.Canonicalise",
