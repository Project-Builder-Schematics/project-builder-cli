...
> **Last updated**: 2026-05-11 (post `/build` + merge of angular-subprocess-adapter via PR #14)
...
| 4 | AngularSubprocessAdapter | L (sensitivity override: subprocess + supply-chain + privilege) | angular-adapter | First concrete `Engine` impl. NDJSON over stdio + embedded `runner.js` via `//go:embed` + SIGTERM/SIGKILL build-tag isolation + injectable Discoverer. 8 domains, 38 REQs, 4 ADRs, 9 existing fitness functions reused; 6 SPIDR slices (incl. integration slice with real Node) | ✅ **DONE** — spec [#11](https://github.com/Project-Builder-Schematics/project-builder-cli/issues/11) closed via PR [#14](https://github.com/Project-Builder-Schematics/project-builder-cli/pull/14); verify-final pass-with-followups (38/38 REQs, 9/9 FFs, race-clean; coverage angular 87.9% / discoverer 79.3% / codec 74.5%); ADRs 016–019 promoted (NDJSON wire / //go:embed runner / SIGTERM→SIGKILL build tags / Discoverer concrete + function injection); 20 followups registered in engram `project/pending-changes`. CI-only fix `ebe4878` post-merge (discoverer host-env isolation via `export_test.go` + gofumpt/goimports idempotency on multi-line `append`). |
